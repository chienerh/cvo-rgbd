...
if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian
  ...
  % 
  temp_PHI_Alpha = PHI'*PHI*beta + sparse(diag(Alpha));
  U		= chol(temp_PHI_Alpha);
%   Ui	= inv(U);
  Ui	= U \ speye(size(U)); % MGJ: using \ instead of inv()
%   SIGMA	= Ui * Ui';
 SIGMA	= temp_PHI_Alpha \ speye(size(U));
  
  % MGJ: Alternative compute posterior covariance
%   SIGMA	= (PHI'*PHI*beta + diag(Alpha)) \ eye(length(Alpha));
  ...
  % 
%   Ui	= inv(U);
  Ui	= U \ eye(size(U)); % MGJ: using \ instead of inv()
  SIGMA	= Ui * Ui';
...
%   betaBASIS_PHI	= beta*BASIS_PHI;			
  ...
  %
  betaBASIS_PHI = beta * BASIS_PHI * Ui;
  S_in		= beta - sum(betaBASIS_PHI .* betaBASIS_PHI, 2);
