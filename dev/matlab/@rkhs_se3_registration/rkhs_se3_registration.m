classdef rkhs_se3_registration < handle
    ...
    methods(Static)
        ...
        function Phi = poly_feature_map(X)
           % Polynomial feature map: https://en.wikipedia.org/wiki/Polynomial_kernel
           Phi = [X.^2, sqrt(2) * X(:,3) .* X(:,2), ...
                sqrt(2) * X(:,3) .* X(:,1), sqrt(2) * X(:,2) .* X(:,1), ...
                sqrt(2*c) * X, c*ones(size(X,1), 1)];            
        end
...
%                 obj.CI = obj.color_inner_product(target.Color, source.Color, obj.color_scale);
                obj.CI = obj.feature_inner_product(target.Color, source.Color, obj.color_scale);
