...
function [omega,v,step] = transform_identity_NR_4_vectorized(cloud_x,cloud_y,param)
...
K = se_kernel(cloud_x.Location, cloud_y.Location, ell, sigma^2);
A = param.C .* K;
...
parfor i = 1:length_x
    partial_omega = 1/c * A(i,:) * ...
        cross(repmat(cloud_x.Location(i,:), length_y,1), cloud_y.Location,2);
    partial_v = 1/d * A(i,:) * ...
        (cloud_y.Location - cloud_x.Location(i,:));
...
xi_z = (cross(OMEGA,cloud_y.Location(:,:),2) + v);
xixi_z = (omega_hat^2 * (cloud_y.Location(:,:)') + omega_hat * v')';
xixixi_z = (omega_hat^3 * (cloud_y.Location(:,:)') + omega_hat^2 * v')';
xixixixi_z = (omega_hat^4 * (cloud_y.Location(:,:)') + omega_hat^3 * v')';
...
parfor i = 1:length_x
    y_i = cloud_x.Location(i,:) - cloud_y.Location;
    beta_i = -sum(xi_z .* y_i,2)/ell^2;
    gamma_i = -1/(2*ell^2)*(normxiz2 + 2 * sum(xixi_z .* y_i,2));
    delta_i = 1/(2*ell^2)*(dotxizxixi + 2 * sum(-xixixi_z .* y_i,2));
    epsil_i = -1/(2*ell^2)*(epsil_const + 2 * sum(xixixixi_z .* y_i,2));
    
    b_i(i) = A(i,:) * beta_i;
    beta2_i = beta_i .* beta_i;
    c_i(i) = A(i,:) * (gamma_i + beta2_i/2);
    beta_gamma_i = beta_i .* gamma_i;
    beta3_i = beta2_i .* beta_i;
    d_i(i) = A(i,:) * (delta_i + beta_gamma_i + beta3_i/6);
    e_i(i) = A(i,:) * (epsil_i + beta_i .* delta_i + ...
...
step = min( min(r((r>0)&(r==real(r)))) , 0.2);
if isempty(step)
    step = 2*1e-1;
end
omega = omega';
