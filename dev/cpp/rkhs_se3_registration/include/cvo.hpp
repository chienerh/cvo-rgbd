...
class cvo{
    ...
    public:
        ...
        cvo(float s_ell_in, float s_sigma_in);
        ~cvo();

        /**
         * @brief initialize new point cloud and extract pcd as matrices
         */
        void set_pcd(const int dataset_seq,const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
         ...
         * @brief run acvo
         */ 
        void run_cvo(const int dataset_seq,const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
                     const bool* mask, string pcd_pth, string pcd_dso_pth);
};
}
#endif  // RKHS_SE3_H
