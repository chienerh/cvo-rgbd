...
namespace cvo{
    ...
    void pcd_generator::get_features(const int feature_type, frame* ptr_fr, point_cloud* ptr_pcd){
        ...
        ptr_pcd->labels = Eigen::MatrixXf::Zero(num_selected,NUM_CLASS);
        int idx = 0;
            ...
            for(int y=0; y<h; ++y){
                for(int x=0; x<w; ++x){
                    ...
                    if(map[y*w+x]!=0 && dep!=0  && !isnan(dep)){
                        ...
                        // std::cout<<"labels: "<<ptr_pcd->labels.row(idx)<<std::endl;
                        ++idx;
                    }
                }
            }
    ...
    void pcd_generator::load_image(const cv::Mat& RGB_img,const cv::Mat& dep_img,MatrixXf_row semantic,frame* ptr_fr){
        ...
        ptr_fr->semantic_labels = semantic;
    }

    void pcd_generator::load_map(frame* ptr_fr, const bool* mask){
        int w = ptr_fr->w;
        int h = ptr_fr->h;
        ...
        for(int y=0; y<h; ++y){
            for(int x=0; x<w; ++x){
                map[y*w+x] = mask[y*w+x];
            }
        }
    }

    void pcd_generator::create_pointcloud(const int feature_type, frame* ptr_fr, point_cloud* ptr_pcd,\
                                            const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
                                            const bool* mask){
        
        int h = ptr_fr->h;
        int w = ptr_fr->w;
        
        load_image(RGB_img,dep_img,semantic,ptr_fr);

        // load_map(ptr_fr,mask);
        // make_pyramid(ptr_fr);
        ...
        // create point cloud in pcl format for visualization/save
    ...
    void pcd_generator::read_pcd_from_file(std::string filename, point_cloud* ptr_pcd){
        std::ifstream infile(filename);
        int num_class = 0;
        infile >> ptr_pcd->num_points;
        infile >> num_class;
        ptr_pcd->features = Eigen::MatrixXf::Zero(ptr_pcd->num_points,NUM_FEATURES);
        Eigen::Vector3f temp_position;
        for (int j = 0; j < ptr_pcd->num_points; j++) {
            int u, v;
            float idepth;
            infile >> u >> v;
            infile >> idepth;
            infile >> ptr_pcd->features(j,0) >> ptr_pcd->features(j,1) >> ptr_pcd->features(j,2) \
                   >> ptr_pcd->features(j,3) >> ptr_pcd->features(j,4);
            infile >> temp_position(0) >> temp_position(1) >> temp_position(2);

            ptr_pcd->features(j,0) = ptr_pcd->features(j,0)/255.0;
            ptr_pcd->features(j,1) = ptr_pcd->features(j,1)/255.0;
            ptr_pcd->features(j,2) = ptr_pcd->features(j,2)/255.0;
            ptr_pcd->features(j,3) = ptr_pcd->features(j,3)/255.0+0.5;
            ptr_pcd->features(j,4) = ptr_pcd->features(j,4)/255.0+0.5;

            ptr_pcd->positions.push_back(temp_position);
            // p.num_semantic_classes = num_class;
            if (num_class > 0) {
                for ( int i = 0; i < num_class; i++) {
                infile >> ptr_pcd->labels(j,i);
                }
            }
        }
        infile.close();
