...
void acvo::se_kernel(point_cloud* cloud_a, point_cloud* cloud_b, \
    ...
    // for(int i=0; i<num_fixed; ++i){
        ...
        Eigen::Matrix<float,NUM_CLASS,1> labels_a = cloud_a->labels.row(i).transpose();
...
void acvo::compute_flow(){
    ...
    // float temp = A.sum()/A.nonZeros();
    // std::cout<<"inner product: "<<temp<<std::endl;
    ...
    // std::cout<<"omega: "<<(omega/omega.norm()).transpose()<<" "<<(v/v.norm()).transpose()<<std::endl;
...
void acvo::set_pcd(const int dataset_seq,const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
    ...
    if(init == false){
        ...
        // pcd_gen.read_pcd_from_file(pcd_pth,ptr_fixed_pcd.get());
    ...
    // pcd_gen.read_pcd_from_file(pcd_pth,ptr_moving_pcd.get());
...
void acvo::run_cvo(const int dataset_seq,const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
                     const bool* mask, string pcd_pth, string pcd_dso_pth){
