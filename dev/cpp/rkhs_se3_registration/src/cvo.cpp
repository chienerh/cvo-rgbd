...
cvo::cvo(float s_ell_in, float s_sigma_in):
    ...
    ell(0.1),             // kernel characteristic length-scale
...
void cvo::se_kernel(const float l, const float s2){
    ...
    // for(int i=0; i<num_fixed; ++i){
        ...
        Eigen::Matrix<float,NUM_FEATURES,1> feature_x = ptr_fixed_pcd->features.row(i).transpose();
        Eigen::Matrix<float,NUM_CLASS,1> labels_a = ptr_fixed_pcd->labels.row(i).transpose();
        ...
        for(size_t j=0; j<nMatches; ++j){
            ...
            if(d2<d2_thres){
                ...
                Eigen::Matrix<float,NUM_FEATURES,1> feature_y = ptr_moving_pcd->features.row(idx).transpose();
                Eigen::Matrix<float,NUM_CLASS,1> labels_b = ptr_moving_pcd->labels.row(idx).transpose();
                d2_color = ((feature_x-feature_y).squaredNorm());
                // std::cout<<"labels_a: "<<labels_a.transpose()<<std::endl;
                // std::cout<<"labels_b: "<<labels_b.transpose()<<std::endl;
                
                // use 4.9 kernel
                // d2_semantic = ((labels_a-labels_b).squaredNorm()); //sift feature
                // d2_semantic = ((labels_b).squaredNorm()); //sift flow

                // use 4.18 kernel
                d_semantic = labels_b.norm(); // sift_flow norm 4.18 l_2 distance
                // d_semantic = ((labels_a-labels_b).norm()); //sift feature 4.18 l_2 distance
                // d_semantic = (labels_a-labels_b).template lpNorm<1>(); //sift feature 4.18 l_1 distance

                if(d2_color<d2_c_thres){
                    k = s2*exp(-d2/(2.0*l*l));
                    ck = c_sigma*c_sigma*exp(-d2_color/(2.0*c_ell*c_ell));
                    // sk = s_sigma*labels_a.dot(labels_b); // inner product
                    // sk = s_sigma*s_sigma*exp(-d2_semantic/(2.0*s_ell*s_ell)); // 4.9 kernel
                    sk = s_sigma*s_sigma*exp(-d_semantic/s_ell); // 4.18 kernel
...
void cvo::set_pcd(const int dataset_seq,const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
                    const bool* mask, string pcd_pth, string pcd_dso_pth){
    ...
    if(init == false){
        ...
        pcd_gen.create_pointcloud(0,ptr_fixed_fr.get(), ptr_fixed_pcd.get(),RGB_img,dep_img,semantic,mask);
    ...
    pcd_gen.create_pointcloud(0,ptr_moving_fr.get(), ptr_moving_pcd.get(),RGB_img,dep_img,semantic,mask);
...
void cvo::run_cvo(const int dataset_seq,const cv::Mat& RGB_img,const cv::Mat& dep_img, const MatrixXf_row& semantic,\
                    const bool* mask, string pcd_pth, string pcd_dso_pth){

    if(init == false){
        set_pcd(dataset_seq,RGB_img, dep_img, semantic, mask, pcd_pth, pcd_dso_pth);
    }
    else{
        set_pcd(dataset_seq,RGB_img, dep_img, semantic, mask, pcd_pth, pcd_dso_pth);
        align();
