...
def main():
    # target file name
    folder_name = "freiburg1_desk"
    file_name = "cvo_poses_qt"

    # Trajectory
    f = open('../'+folder_name+'/'+file_name+'.txt', 'r')
    x = f.readlines()
    f.close()
    ...
    f = open('../'+folder_name+'/'+'assoc.txt', 'r')
    t = f.readlines()
    f.close()
    ...
    x = x[:, perm]
    np.savetxt('../'+folder_name+'/'+file_name+'_t.txt', x, delimiter = ' ', fmt = '%.6f')
